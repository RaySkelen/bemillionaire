...
const dataReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case REMOVE_USED_QUESTION: {
            return { ...state, questions: [...state.questions.slice(0, action.index), ...state.questions.slice(action.index + 1)] };
        }
        default: {
            return state
        }
...
export const removeQuestion = (index) => ({ type: REMOVE_USED_QUESTION, index });
